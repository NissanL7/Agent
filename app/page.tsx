...
export default function Home(){
 const [chats,setChats]=useState<Chat[]>([]); const [activeId,setActiveId]=useState(''); const [task,setTask]=useState(''); const [provider,setProvider]=useState<Provider>('gpt'); const [orchestrate,setOrchestrate]=useState(true); const [busy,setBusy]=useState(false); const [sidebar,setSidebar]=useState(false); const [gmail,setGmail]=useState(false); const [about,setAbout]=useState(false)
 ...
 return <main className="appShell">
  ...
  <section className="chatArea">
   ...
   <div className="messages">
    ...
    {active?.messages.map((m,i)=><div className={`messageRow ${m.role}`} key={i}><div className="messageAvatar">{m.role==='user'?'N':'✦'}</div><div className="messageBody"><div className="messageMeta">{m.role==='user'?'You':m.team?'Cross-model':m.model||'AI'}</div><div className="markdown">{renderMarkdown(m.content)}</div></div></div>)}
   ...
   <div className="composerWrap"><div className="composerBox"><textarea value={task} onChange={e=>setTask(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();run()}}} placeholder={orchestrate?'Ask the cross-model team anything…':`Ask ${models.find(m=>m[2]===provider)?.[0]||'AI'} anything…`} rows={1}/><div className="composerTools"><button className="tool">＋</button><div className="modeText">{orchestrate?'3 models: reason + review + synthesize':`Single: ${models.find(m=>m[2]===provider)?.[0]||'AI'}`} · Enter to send</div><button className="send" disabled={!task.trim()||busy} onClick={run}>↑</button></div></div><div className="privacy">Hugging Face Inference Providers powers the connected models.</div></div>
  </section>
